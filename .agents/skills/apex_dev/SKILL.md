...
| **`HTTP 404` on APEX Builder / `apex_admin`** | ORDS `pool.xml` missing PL/SQL gateway enablement, OR APEX install falsely skipped due to `all_users LIKE 'APEX_%'` matching proxy schema (`APEX_PROXY_SCHEMA`). | Configure `<entry key="plsql.gateway.mode">proxied</entry>` and `<entry key="feature.apex">true</entry>`. In-DB APEX check must query `dba_registry WHERE comp_id = 'APEX' AND status IN ('VALID', 'UPGRADED')`. |
| **`Killed` (OOM 137) during APEX Patch / Install** | SQLcl (JVM) executed inside 3GB container alongside Oracle DB exceeds memory limit during metadata loading (`catpatch.sql` / `apxins.sql`). | **Rule 6 Exemption:** In-container APEX core engine and patch execution must use native C-binary `$ORACLE_HOME/bin/sqlplus -s / as sysdba` (~15MB RAM, 0 JVM overhead). |
