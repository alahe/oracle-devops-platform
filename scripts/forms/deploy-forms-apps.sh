...
echo -e "\n${CYAN}==================================================================${NC}"
echo -e "${BOLD}📦 ORACLE FORMS 14c RAKENDUSTE TARNE (DEPLOY FORMS APPS)${NC}"
echo -e "${CYAN}==================================================================${NC}"
...
if [ -d "$SOURCE_DIR" ]; then
  ...
  for f in "$SOURCE_DIR"/*.fmx "$SOURCE_DIR"/*.mmx "$SOURCE_DIR"/*.plx; do
    [ -f "$f" ] || continue
    bname=$(basename "$f")
    echo "   ├─ Kopeerin rakenduse faili: ${CYAN}$bname${NC}"
  ...
  echo -e "✅ ${GREEN}Tarnitud ${count} Forms rakenduse faili kausta /u01/oracle/forms_apps!${NC}"
else
  echo -e "⚠️  Kausta $SOURCE_DIR ei leitud."
fi
echo -e "${CYAN}==================================================================${NC}\n"
