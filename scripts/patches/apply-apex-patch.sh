...
if [ "$EXEC_MODE" = "CLIENT" ]; then
  ...
  if [ -n "$OTHER_CONTAINERS" ]; then
    echo "ℹ️ Vabastan mälumahtu APEX patchi ajaks: peatan ajutiselt konteinerid: $OTHER_CONTAINERS..."
...
EOF
  ...
  print_progress "Paigaldan APEX patchi (${CONTAINER_NAME})" 0 60
  while kill -0 $SQL_PID 2>/dev/null; do
    sleep "$POLL_INTERVAL"
    ELAPSED=$((ELAPSED + POLL_INTERVAL))
    print_progress "Paigaldan APEX patchi (${CONTAINER_NAME})" "$ELAPSED" 60
  done
  wait $SQL_PID
  clear_progress_line
  restore_cursor
