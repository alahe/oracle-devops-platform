...
CONFIG_FILE="$WORKSPACE_DIR/config/repository.env"
...
if [ -z "$PATCH_ZIP" ]; then
  echo "Error: Please provide path to APEX patch zip file as an argument!"
  echo "Usage: ./scripts/apply-apex-patch.sh patches/<patch_file>.zip [--no-ords]"
  exit 1
fi

if [ ! -f "$PATCH_ZIP" ]; then
  echo "Error: File '$PATCH_ZIP' not found!"
  exit 1
fi
...
if [ "$IS_CONTAINER_AVAIL" = "true" ]; then
  ...
  if [ -z "$PATCH_SQL_FILE" ]; then
    echo "❌ Error: Could not find patch installation script (catpatch.sql or apxpatch.sql) in container!"
    exit 1
  fi
  ...
  echo "Found patch location in container: $PATCH_DIR (script: $PATCH_SCRIPT_NAME)"
  ...
  print_sub_header "1" "Unpacking APEX patch on host: $PATCH_ZIP..." "step6_apex_copy_container_seconds" "step6_apex_copy_container_seconds" "5s"
  ...
  if [ -z "$PATCH_SQL_FILE" ]; then
    echo "❌ Error: Could not find patch installation script (catpatch.sql or apxpatch.sql) in zip file!"
    exit 1
  fi
  ...
  echo "Found patch location on host: $PATCH_DIR (script: $PATCH_SCRIPT_NAME)"
  PSTEP1_TIME=$(format_duration $(($(date +%s) - PSTEP1_START)))
...
EOF
  ...
  print_progress "Applying APEX patch (${CONTAINER_NAME})" 0 60
  while kill -0 $SQL_PID 2>/dev/null; do
    sleep "$POLL_INTERVAL"
    ELAPSED=$((ELAPSED + POLL_INTERVAL))
    print_progress "Applying APEX patch (${CONTAINER_NAME})" "$ELAPSED" 60
  done
  wait $SQL_PID
  clear_progress_line
  restore_cursor
  ...
  if [ -n "$OTHER_CONTAINERS" ]; then
    echo "ℹ️  Restarting stopped containers: $OTHER_CONTAINERS..."
    podman start $OTHER_CONTAINERS >/dev/null 2>&1 || true
  fi
echo ""
echo "✅ APEX patch SQL execution completed successfully."
...
echo -e "⏱  [Patch step 3 completed (SQL/PLSQL execution): ${YELLOW}$PSTEP3_TIME${NC}]"

# ----------------------------------------------------------------------------
# 4. Updating static assets and images
# ----------------------------------------------------------------------------
PSTEP4_START=$(date +%s)
  print_sub_header "3" "Updating local and ORDS static APEX image assets..." "" "" "2s"
copy_patch_images_to_volume
