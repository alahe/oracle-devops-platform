...
[ ! -d "$PATCH_DIR" ] && PATCH_DIR="$WORKSPACE_DIR/patches"
...
# Scan for Publisher patch ZIPs in binaries/publisher/patches/ or patches/
PATCH_FILES=$(find "$WORKSPACE_DIR/binaries/publisher/patches" -maxdepth 2 \( -name "*publisher*.zip" -o -name "*oas*.zip" -o -name "p*.zip" \) 2>/dev/null || true)
if [ -z "$PATCH_FILES" ] && [ -d "$WORKSPACE_DIR/patches" ]; then
  PATCH_FILES=$(find "$WORKSPACE_DIR/patches" -maxdepth 2 \( -name "*publisher*.zip" -o -name "*oas*.zip" -o -name "p*.zip" \) 2>/dev/null || true)
fi

if [ -z "$PATCH_FILES" ]; then
  echo "ℹ️  No Publisher OPatch ZIP packages found in binaries/publisher/patches/ (or patches/) directory." | tee -a "$LOG_FILE"
  echo "    Place OPatch ZIP files (e.g. p39179920_publisher.zip) in binaries/publisher/patches/ to apply automated patches." | tee -a "$LOG_FILE"
  exit 0
fi

PRIMARY_CONTAINER="${PUBLISHER_CONTAINER_NAME:-app-publisher}"
