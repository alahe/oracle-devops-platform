...
apply_profile_users() {
  ...
  [ -z "$target_container" ] && target_container=$(get_active_db_instances 2>/dev/null | head -n 1 | cut -d'|' -f1 || echo "db-dev-full")
  ...
  c_short=$(echo "$target_container" | sed 's/^db-//' | tr '[:lower:]' '[:upper:]')

  DBA_ADMIN_PASSWORD=$("$SCRIPT_DIR/../get-password.sh" "DB_${c_short}_DBA_ADMIN" 2>/dev/null | grep "Password:" | sed $'s/\x1b\\[[0-9;]*m//g' | cut -d':' -f2- | tr -d ' \r' || true)
  [ -z "$DBA_ADMIN_PASSWORD" ] && DBA_ADMIN_PASSWORD=$(get_container_secret "$target_container" "dba_admin_password" 2>/dev/null || true)
  [ -z "$DBA_ADMIN_PASSWORD" ] && DBA_ADMIN_PASSWORD=$(LC_ALL=C tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 20 2>/dev/null)

  TEST_DEV_PASSWORD=$("$SCRIPT_DIR/../get-password.sh" "DB_${c_short}_DEV" 2>/dev/null | grep "Password:" | sed $'s/\x1b\\[[0-9;]*m//g' | cut -d':' -f2- | tr -d ' \r' || true)
  [ -z "$TEST_DEV_PASSWORD" ] && TEST_DEV_PASSWORD=$(get_container_secret "$target_container" "test_dev_password" 2>/dev/null || true)
  [ -z "$TEST_DEV_PASSWORD" ] && TEST_DEV_PASSWORD=$(LC_ALL=C tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 20 2>/dev/null)

  TEST_VIEWER_PASSWORD=$("$SCRIPT_DIR/../get-password.sh" "DB_${c_short}_VIEWER" 2>/dev/null | grep "Password:" | sed $'s/\x1b\\[[0-9;]*m//g' | cut -d':' -f2- | tr -d ' \r' || true)
  [ -z "$TEST_VIEWER_PASSWORD" ] && TEST_VIEWER_PASSWORD=$(get_container_secret "$target_container" "test_viewer_password" 2>/dev/null || true)
  [ -z "$TEST_VIEWER_PASSWORD" ] && TEST_VIEWER_PASSWORD=$(LC_ALL=C tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 20 2>/dev/null)

  TEST_WEB_PASSWORD=$(get_container_secret "$target_container" "test_web_password" 2>/dev/null || true)
  [ -z "$TEST_WEB_PASSWORD" ] && TEST_WEB_PASSWORD=$(LC_ALL=C tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 20 2>/dev/null)

  APEX_SCHEMA_PASSWORD=$("$SCRIPT_DIR/../get-password.sh" "DB_${c_short}_SCHEMA" 2>/dev/null | grep "Password:" | sed $'s/\x1b\\[[0-9;]*m//g' | cut -d':' -f2- | tr -d ' \r' || true)
  [ -z "$APEX_SCHEMA_PASSWORD" ] && APEX_SCHEMA_PASSWORD=$(get_container_secret "$target_container" "apex_schema_password" 2>/dev/null || true)
  ...
  APEX_LISTENER_PASSWORD=$(get_container_secret "$target_container" "ords_listener_password" 2>/dev/null || true)
  [ -z "$APEX_LISTENER_PASSWORD" ] && APEX_LISTENER_PASSWORD=$(get_container_secret "$target_container" "apex_schema_password" 2>/dev/null || true)
