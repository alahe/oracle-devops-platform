...
echo -e "${YELLOW}⌛ Ootan kuni kõik andmebaasid on terved ja PDB avatud (READ WRITE)...${NC}"
...
for container in "${CONTAINERS_TO_CHECK[@]}"; do
  ...
  while true; do
    ...
    if [ "$c_status" = "running" ]; then
      if [ "$h_status" = "healthy" ] || [ "$h_status" = "none" ]; then
        if podman exec "$container" sh -c "[ -f /u01/container_state/.installed_ords ]" 2>/dev/null; then
          ...
          pdb_check=$(echo -e "SET HEADING OFF FEEDBACK OFF;\nSHOW PDBS;\nEXIT;" | podman exec -i "$container" sqlplus -s -L / as sysdba 2>/dev/null || true)
    ...
    if [ "$is_healthy" = "true" ]; then
      clear_progress_line
      echo -e "   ✅ Konteiner ${GREEN}${container}${NC} on TERVE ja PDB avatud READ WRITE!"
      break
    fi
    ...
    print_progress "Ootan konteinerit $container" "$WAIT_COUNT" "$MAX_WAIT"
    ...
    if [ "$AUTO_RESTARTED" = "false" ] && [ $WAIT_COUNT -ge 45 ] && [ "$c_status" = "running" ]; then
      if ! echo -e "SHOW PDBS;\nexit;" | podman exec -i "$container" sqlplus -s -L / as sysdba 2>/dev/null | grep -q "READ WRITE"; then
        AUTO_RESTARTED=true
        clear_progress_line
        echo -e "   ⚠️  Konteiner ${container} töötab, aga DB instants ei vasta (zombie olek). Teostan automaatse iseparaneva taaskäivituse (podman restart)..."
    ...
    if [ $WAIT_COUNT -ge $MAX_WAIT ]; then
      clear_progress_line
      echo -e "\n${RED}❌ Viga: Konteiner $container ei saavutanud 'healthy' ja 'READ WRITE' olekut $MAX_WAIT sekundi jooksul!${NC}"
      exit 1
    fi
  done
done
