...
echo -e "${CYAN}==================================================================${NC}"
echo -e "${YELLOW}🔍 Kontrollin süsteemi ressursside ja Podman keskkonna valmisolekut...${NC}"
echo -e "${CYAN}==================================================================${NC}"
...
if command -v podman >/dev/null 2>&1; then
  ...
  if [[ "$FREE_DISK_MB" =~ ^[0-9]+$ ]]; then
    ...
    if [ "$FREE_DISK_GB" -lt 8 ]; then
      ...
      echo -e "${YELLOW}   💡 Soovitus: Vabasta kiiresti kettaruumi: podman system prune -a${NC}"
    elif [ "$FREE_DISK_GB" -lt 15 ]; then
      echo -e "${YELLOW}⚠️  HOIATUS: Podman VM vaba kettaruum on madal: ${FREE_DISK_GB} GB (Soovitatav vähemalt 15 GB)${NC}"
      echo -e "${YELLOW}   💡 Soovitus: Vabasta kettaruumi käsuga: podman system prune -a${NC}"
    else
      echo -e "   ├─ Podman VM Vaba Kettaruum: ${GREEN}${FREE_DISK_GB} GB${NC}"
    fi
  fi
fi
...
echo -e "   ├─ Masina Kogu RAM Mälumaht: ${GREEN}${TOTAL_RAM_GB} GB${NC}"
...
if [ "$TOTAL_RAM_GB" -lt 8 ] || [ "${FORCE_SEQUENTIAL:-true}" = "true" ]; then
  ...
  echo -e "   └─ Töörežiim: ${YELLOW}Range Järjestikune Paigaldus (Strict Sequential Mode)${NC} (Tagab 100% stabiilsuse ilma OOM seiskumiseta)"
else
  export STRICT_SEQUENTIAL_MODE=false
  echo -e "   └─ Töörežiim: ${GREEN}Paralleelne Paigaldus (Parallel Mode Enabled)${NC}"
fi
