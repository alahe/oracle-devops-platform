...
5. **Samm 4 (Variant 4 - Jooksvalt genereeritud Iseallkirjastatud sertifikaat - `config/certs/self_signed/`):**
   * ⚠️ **Madalaim tase (Untrusted fallback):** Tekitab brauseris *"Not Secure"* hoiatuse. Lubatud ainult siis, kui `TLS_ALLOWED_LEVEL=permissive`.
...
* **Windows (PowerShell):**
  * [`trust-local-cert.ps1`](trust-local-cert.ps1): Lisab CA kasutaja isiklikku hoidlasse `Cert:\CurrentUser\Root` ilma administraatori õigusteta.
* **Windows (CMD):**
...
* **Platvormiülene Puhastus (All OS):**
  * [`clean-certs.sh`](clean-certs.sh) (CLI wrapper: `./scripts/clean-certs.sh`): Puhastab vanad sertifikaadid nii OS võtmehoidjast kui kettalt.
